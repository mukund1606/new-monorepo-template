...
const MAX_PRELOAD_BYTES = env.STATIC_PRELOAD_MAX_BYTES; // 5MB default

// Parse comma-separated include patterns (no defaults)
const INCLUDE_PATTERNS = env.STATIC_PRELOAD_INCLUDE.split(",")
  .map((s) => s.trim())
  .filter(Boolean)
  .map(globToRegExp);

// Parse comma-separated exclude patterns (no defaults)
const EXCLUDE_PATTERNS = env.STATIC_PRELOAD_EXCLUDE.split(",")
  .map((s) => s.trim())
  .filter(Boolean)
  .map(globToRegExp);

// Verbose logging flag
const VERBOSE = env.STATIC_PRELOAD_VERBOSE === "true";

// Optional features
const ENABLE_ETAG = env.STATIC_PRELOAD_ETAG === "true";
const ENABLE_GZIP = env.STATIC_PRELOAD_GZIP === "true";
const GZIP_MIN_BYTES = env.STATIC_PRELOAD_GZIP_MIN_BYTES; // 1KB
const GZIP_TYPES = env.STATIC_PRELOAD_GZIP_TYPES.split(",")
  .map((v) => v.trim())
  .filter(Boolean);
...
function computeEtag(data: Uint8Array): string {
  ...
  return `W/"${hash.toString(16)}-${String(data.byteLength)}"`;
}
...
function buildCompositeGlob(): Bun.Glob {
  const raw = (env.STATIC_PRELOAD_INCLUDE || "")
    .split(",")
    .map((s) => s.trim())
    .filter(Boolean);
...
async function buildStaticRoutes(clientDir: string): Promise<PreloadResult> {
  ...
  if (INCLUDE_PATTERNS.length > 0) {
    console.log(`   Include patterns: ${env.STATIC_PRELOAD_INCLUDE}`);
  }
  if (EXCLUDE_PATTERNS.length > 0) {
    console.log(`   Exclude patterns: ${env.STATIC_PRELOAD_EXCLUDE}`);
  }
