...
// Configuration
const SERVER_PORT = Number(serverEnv.PORT);
...
const MAX_PRELOAD_BYTES = serverEnv.ASSET_PRELOAD_MAX_SIZE;

// Parse comma-separated include patterns (no defaults)
const INCLUDE_PATTERNS = serverEnv.ASSET_PRELOAD_INCLUDE_PATTERNS.split(",")
  .map((s) => s.trim())
  .filter(Boolean)
  .map((pattern: string) => convertGlobToRegExp(pattern));

// Parse comma-separated exclude patterns (no defaults)
const EXCLUDE_PATTERNS = serverEnv.ASSET_PRELOAD_EXCLUDE_PATTERNS.split(",")
  .map((s) => s.trim())
  .filter(Boolean)
  .map((pattern: string) => convertGlobToRegExp(pattern));

// Verbose logging flag
const VERBOSE = serverEnv.ASSET_PRELOAD_VERBOSE_LOGGING === "true";

// Optional ETag feature
const ENABLE_ETAG = serverEnv.ASSET_PRELOAD_ENABLE_ETAG === "true";

// Optional Gzip feature
const ENABLE_GZIP = serverEnv.ASSET_PRELOAD_ENABLE_GZIP === "true";
const GZIP_MIN_BYTES = serverEnv.ASSET_PRELOAD_GZIP_MIN_SIZE; // 1KB
const GZIP_TYPES = serverEnv.ASSET_PRELOAD_GZIP_MIME_TYPES.split(",")
  .map((v) => v.trim())
  .filter(Boolean);
...
function createCompositeGlobPattern(): Bun.Glob {
  const raw = serverEnv.ASSET_PRELOAD_INCLUDE_PATTERNS.split(",")
    .map((s) => s.trim())
    .filter(Boolean);
...
async function initializeStaticRoutes(clientDirectory: string): Promise<PreloadResult> {
  ...
  if (VERBOSE) {
    ...
    if (INCLUDE_PATTERNS.length > 0) {
      console.log(`Include patterns: ${serverEnv.ASSET_PRELOAD_INCLUDE_PATTERNS}`);
    }
    if (EXCLUDE_PATTERNS.length > 0) {
      console.log(`Exclude patterns: ${serverEnv.ASSET_PRELOAD_EXCLUDE_PATTERNS}`);
    }
  }
